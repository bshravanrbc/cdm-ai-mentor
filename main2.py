...
if "questions" in st.session_state:
    ...
    for i, q in enumerate(questions):
        st.markdown(f"**प्रश्न क्रमांक {i+1}: {q['question']}**")
        user_choice = st.radio(f"Choose your answer: (अपना उत्तर चुनें:)", q['options'], key=f"q{i}")
        user_answers.append(user_choice)

    if st.button(lbl["submit"], key="submit_quiz (प्रश्नोत्तरी सबमिट करें)"):
        ...
        for i, res in enumerate(results['details']):
            st.markdown(f"### प्रश्न क्रमांक {i+1}: {res['question']}")
